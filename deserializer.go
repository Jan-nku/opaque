...
func (d *Deserializer) RegistrationRequest(registrationRequest []byte) (*message.RegistrationRequest, error) {
	...
	username := make([]byte, len(registrationRequest)-d.conf.OPRF.Group().ElementLength())
	copy(username, registrationRequest[d.conf.OPRF.Group().ElementLength():])
	return &message.RegistrationRequest{BlindedMessage: blindedMessage, UserName: username}, nil
}
...
func (d *Deserializer) KE1(ke1 []byte) (*message.KE1, error) {
	...
	if err := epku.Decode(ke1[d.conf.OPRF.Group().ElementLength()+d.conf.NonceLen : 2*d.conf.OPRF.Group().ElementLength()+d.conf.NonceLen]); err != nil {
		return nil, errInvalidClientEPK
	}

	username := make([]byte, len(ke1)-(2*d.conf.OPRF.Group().ElementLength()+d.conf.NonceLen))
	copy(username, ke1[2*d.conf.OPRF.Group().ElementLength()+d.conf.NonceLen:])
