...
func (s *Server) Response(
	...
	ke1 *message.KE1, //口令盲化值、EpkU、NonceU
	response *message.CredentialResponse, //OPRF二次加密值、加密服务器公钥和envelope使用的随机数和加密结果
	options Options, //nil
) *message.KE2 {
	//设置values(ephemeralSecretKey, nonce), 返回epk
	...
	//调用3dh协议生成sessionSecret 以及 mac(serverMac, clientMac)
	//ikm = EpkU ^ s.ephemeralSecretKey || EpkU ^ serverSecretKey || clientPublicKey ^ s.ephemeralSecretKey
	//ikm = g ^ esku ^ esks || g ^ esku ^ sks ^ || g ^ sku ^ esks
	ikm := k3dh(
		ke1.EpkU,
