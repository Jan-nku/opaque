...
func Store(
	...
	//nonce := creds.EnvelopeNonce = nil
	nonce := creds.EnvelopeNonce
	if nonce == nil {
		nonce = internal.RandomBytes(conf.NonceLen)
	}

	//getPubkey 获取客户端公钥, randomizedPwd + nonce --> sk
	...
	//auth = MAC_authkey(nonce, ctc), authkey = hash(randomizedPwd, nonce)
	auth := authTag(conf, randomizedPwd, nonce, ctc)

	//randomizedPwd, nonce --> export
