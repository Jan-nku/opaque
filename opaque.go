...
func (c *Configuration) Generate() (t1, t2, gs *group.Element) {
	scalar1 := c.OPRF.Group().NewScalar().Random()
	t1 = c.OPRF.Group().Base().Multiply(scalar1)
	scalar2 := c.OPRF.Group().NewScalar().Random()
	t2 = c.OPRF.Group().Base().Multiply(scalar2)
	scalar3 := c.OPRF.Group().NewScalar().Random()
	gs = c.OPRF.Group().Base().Multiply(scalar3)
...
// TODO: opaque.Configuration converts to internal.Configuration
